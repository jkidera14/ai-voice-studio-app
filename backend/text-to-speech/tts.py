...
app = modal.App("ai-voice-studio-rakide")
...
volume = modal.Volume.from_name(
    "hf-cache-ai-voice-studio", create_if_missing=True)
...
@app.cls(
    ...
    volumes={
        ...
        "/s3-mount": modal.CloudBucketMount("ai-voice-studio-rakide", secret=s3_secret)
    },
...
class TextToSpeechServer:
    ...
    def load_model(self):
        ...
        @modal.fastapi_endpoint(method="POST", requires_proxy=True)
        def generate_speech(self, request: TextToSpeechRequest) -> TextToSpeechResponse:
            ...
            torchaudio.save(buffer, wav_cpu, self.model.sr,
                            format="wav")  # Save as WAV
            buffer.seek(0)  # Reset buffer position to start
            audio_bytes = buffer.read()  # Read the the bytes from the beginning

            # Generate a unique S3 key for the output audio folder named "tts"
